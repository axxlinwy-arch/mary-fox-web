...
function GalleryLightbox({
  ...
  return (
    ...
    >
      ...
      >
        {/* Тот же src — браузер берёт из кэша карусели, без повторной загрузки */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={src}
          alt={alt}
...
export function GalleryMarquee() {
  ...
  useAnimationFrame((_time, delta) => {
    if (!autoPlay || isDragging || selectedKey !== null) return;
    const loop = loopWidthRef.current;
    if (loop <= 0) return;
    x.set(wrapOffset(x.get() - (AUTO_SCROLL_PX_PER_SEC * delta) / 1000, loop));
  });
  ...
  const handleDragEnd = () => {
    x.set(wrapOffset(x.get(), loopWidthRef.current));
    setIsDragging(false);
    if (selectedKey === null) resumeAutoPlay();
  };
  ...
  const renderSet = (keyPrefix: string, eager: boolean) =>
    GALLERY.map((item, index) => (
      <GalleryCard
        ...
        onOpen={() => setSelectedKey(item.imageKey)}
      />
    ));
  ...
  return (
    <section className="overflow-hidden border-y border-accent/10 py-16 md:py-20">
      ...
      <div className="relative select-none">
        <div className="pointer-events-none absolute bottom-0 left-0 top-0 z-10 w-16 bg-gradient-to-r from-black to-transparent md:w-20" />
        <div className="pointer-events-none absolute bottom-0 right-0 top-0 z-10 w-16 bg-gradient-to-l from-black to-transparent md:w-20" />

        <motion.div
          className="flex w-max cursor-grab gap-4 active:cursor-grabbing md:gap-6"
          style={{ x, willChange: isDragging ? "transform" : "auto" }}
          drag="x"
          dragElastic={0.02}
          dragMomentum={false}
